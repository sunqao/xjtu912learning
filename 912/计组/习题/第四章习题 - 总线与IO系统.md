...
这里CPU的主频是50MHz，那就说明CPU的一个时钟周期是$\frac{1}{50MHz}s$

一般而言CPU一次查询需要两条指令，即都状态寄存器指令和测试判断指令

所以在1s中内CPU在查询键盘的时候的指令数为：
$$
5*2
$$
...
所以查询键盘所占的比率为：
$$
\frac{5*2* 5}{50M} * 100 \% = 0.0001\%
$$
由于CPU每次查询硬盘一次就交换一个字节，而硬盘的传输速率是5MB/s，这就说明1s中CPU查询了5M次，所以同理可以得到查询硬盘的比率：
$$
\frac{5M * 2 * 5}{50M} * 100 \% = 100 \%
$$
所以可以看到CPU查询硬盘所占用的时间比率是100%，这就说明CPU对硬盘的控制不适用于程序查询方式，否则CPU全去查询硬盘去了，非常影响CPU本身的效率

## 例 4.5

![image-20240922155722331](https://typora-1310242472.cos.ap-nanjing.myqcloud.com/typora_img/image-20240922155722331.png)

这里的数据传送就是中断时间，准备时间就是CPU的1s中的总时间，同上一题一样，这里同样可以直接用时钟周期个数来做

首先是一次中断占用的时钟周期个数：
$$
(8 + 2) * 5 = 50
$$
用户是5键/s可以认为是键盘每秒中断5次，因此键盘每s中断所占的时钟周期个数就是50 * 5 = 250个
